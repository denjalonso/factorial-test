...
export class HostedOnboarding extends AggregateRoot {
	...
	static fromPrimitives(plainData: { id: string; status: OnboardingStatus; user: {id: string, name: string}}): HostedOnboarding {
		return new HostedOnboarding(new HostedOnboardingId(plainData.id), plainData.status, User.fromPrimitives(plainData.user));
	}
